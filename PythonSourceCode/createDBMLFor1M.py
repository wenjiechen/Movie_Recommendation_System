...
def createTables():
    ...
    cursor = conn.cursor()  
    cursor.execute(""" DROP DATABASE IF EXISTS `MoiveLens1M`""")
    cursor.execute("""CREATE DATABASE `MoiveLens1M`""")
    conn.select_db('MoiveLens1M') 
...
def importRatings():
    fileName = 'D:/dataset/ml-1m/ratings.dat'
    conn = MySQLdb.connect(host='localhost', user='root', passwd='1234')
    cursor = conn.cursor()
    conn.select_db('MoiveLens1M') 
    f= open(fileName)
...
def exportToCSV():
    conn = MySQLdb.connect(db='MoiveLens1M', host='localhost', user='root', passwd='1234')  
    cursor = conn.cursor()
    fileName = "'D:/dataset/ml-1m/userItemRating1M.csv'"
